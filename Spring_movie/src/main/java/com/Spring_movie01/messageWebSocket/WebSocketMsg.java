...
import com.Spring_movie01.testWebSocket.TestMsgDto;
import com.google.gson.Gson;

public class WebSocketMsg extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		String LoginMid = (String)connectMap.get("loginId");
		
		if(LoginMid == null) {
		// 로그인 아이디가 없을 경우 실행
			
			LoginMid = "비회원_"+session.getId();
			
		}else {
			
			LoginMid = LoginMid+"_"+session.getId();
			// 로그인 아이디가 있을 경우 랜덤으로 생성된 ID를 
		}
		Gson gson = new Gson();
		TestMsgDto msgDto = new TestMsgDto();
		msgDto.setMsgUserId(LoginMid);
		...
		for(int i = 0; i<sessionList.size(); i++) {
			...
			if(conUserId == null) {
				conUserId = "비회원"+sessionList.get(i).getId();
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		Gson gson = new Gson();
		TestMsgDto msgDto = gson.fromJson(message.getPayload(), TestMsgDto.class);
		...
		for(int i = 0; i < sessionList.size(); i++) {
			...
			if(!sessionList.get(i).getId().equals(session.getId())) {
				...
				sessionList.get(i).sendMessage(new TextMessage(gson.toJson(msgDto)));
				// 다른 session에 메세지를 보낸다. ( 새로운 TextMaessage객체에 전송받은 메세지를 담아서 보낸다. )
			}
		}
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		// 페이지를 나가기전까지 session 계속 유지가 된다.	
		Map<String, Object> connectMap = session.getAttributes();
		String LoginMid = (String)connectMap.get("LoginMid");
